...
class Indexing:
    ...
    def prepareInsertQuery(self):
        self.query_insert = 'INSERT INTO TF(WORD,'
        for i in xrange(1, self.N):
            self.query_insert = self.query_insert + 'DOC' + str(i) + ', '
        self.query_insert = self.query_insert + 'DOC' + str(self.N)
        self.query_insert = self.query_insert + ") VALUES('"
    ...
    def removeStopWords(self, wordlist):
        with open(self.stopWordFile) as F_Stop:
            stopWordList = set(F_Stop.read().split(' '))
    ...
    def TF_insert(self, fid, word, count_total, word_freq):
        ...
        try:
            ...
            if not present:
                ...
                query_insert = self.query_insert+ word + "',"
    ...
    def IDF_insert(self):
        ...
        for row in cursor:
            ...
            for ele in row:
                if ele == 0:
                    c -= 1
            insert_q = "INSERT INTO IDF(WORD,IDF_VALUE) VALUES('%s'," % row[0]
            assert c != 0
            value = math.log(self.N / float(c))
            insert_q += ('%f )' % value)
